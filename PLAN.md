...
- **출력 canvas longerSide 2560 정규화**: `TARGET_LONGER_SIDE = 2560`. scale = 2560 / max(imgW, imgH). 모든 이미지에서 표/글자 픽셀 크기 일관화. 원본 upscale(작은 이미지) 시 blur 용인.
- 작업원 태그 규칙
  - 작업자/서류: 메인 폼에 선택된 전체 작업원 이름 공백 연결
  - 차대비: 해당 루프의 1명
- **가변 폭 테이블**: 모든 행의 라벨/값을 `ctx.measureText`로 측정, 최대값 기준으로 `labelColW`/`valueColW` 동적 산출. `tableW = labelColW + valueColW`, 상한 95% imgW. 초과 시 `truncateText` clip.

### 5.6 저장 + 밴드 앱 열기

- **역순 저장**: 밴드 댓글 첨부 시 사진첩이 최신순으로 정렬됨. 저장 순서를 역순으로 하면 밴드에서 원래 순서로 표시. 파일명 suffix도 역순 번호 재부여 (마지막 저장 = `_1`).
- "저장 + 밴드 열기" 버튼 클릭 시:
  1. 합성 사진 로컬 다운로드 (`<a download>` 트릭) — 역순 배열, 파일명 재번호
