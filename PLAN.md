...
**2단계: 작업자 사진**
- 프롬프트 카드: "작업원 사진 고르세요" + **[선택]** / **[건너뛰기]** 버튼 2개
- [선택] 클릭 → `<input type="file" accept="image/*">` 프로그래매틱 트리거 → **앨범에서 선택** (`capture` 속성 없음)
- 파일 선택 완료 → 3단계
- [건너뛰기] 클릭 → skip (생성 안 함) → 3단계
- (참고) 모바일에서 파일 선택창 "취소" 시 `change` 이벤트 미발화 플랫폼이 있어서 Y/N confirm 대신 UI 버튼 방식 채택

**3단계: 서류 사진** — 2단계와 동일 흐름 ("서류 사진 고르세요")

**4단계: 차대비 사진 (작업원별 루프)**
- 현재 메인 폼에 선택된 작업원 순서대로 반복:
  - 프롬프트 카드: "{이름} 차대비 사진 고르세요" + [선택] / [건너뛰기]
  - [선택] → 파일 선택 → 다음 작업원
  - [건너뛰기] → skip → 다음 작업원

**※ 재클릭 시**: "자동 입히기" 버튼을 플로우 진행 중 다시 누르면 `confirm("진행 중인 자동 입히기를 초기화하시겠습니까?")` 네이티브 확인 → 확인 시 상태 리셋 후 1단계부터 재시작. (명시 버튼 클릭이라 플랫폼 이벤트 문제 없음)
